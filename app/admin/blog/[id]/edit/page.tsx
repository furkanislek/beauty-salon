  ...
  useEffect(() => {
    const loadPost = async () => {
      try {
        ...
        if (!data) throw new Error("Blog yazısı bulunamadı");
        ...
        setFormData({
          ...
          category: post.category || "Cilt Bakımı",
          author_name: post.author_name || "",
          read_time: post.read_time || "5 dk",
    ...
    loadPost();
    // supabase instance'ı stable olduğundan dependency'de güvenle kullanabiliriz
  }, [id, supabase]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const updateData: BlogPostUpdate = {
        ...
        image_url: (imageUrl || formData.image_url) as BlogPostRow["image_url"],
      ...
      const blogPostsTable = supabase.from("blog_posts") as any;
      const { error: updateError } = await blogPostsTable
        .update(updateData)
        .eq("id", id);
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="mb-8">
          ...
          <p className="text-gray-600 mt-1">
            Blog yazısı içeriğini güncelleyin
          </p>
        </div>
